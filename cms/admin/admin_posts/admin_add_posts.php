...
if(isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') { 
    ...
    if (empty($post_image)) {
        ...
        $cms_dir = "./admin_upload_images/$post_image";
        move_uploaded_file($post_image_temp, $cms_dir);
    }

    // Checking if the post exists
    if (post_duplicate($cms_pdo, "posts", "post_title", ucfirst($post_title)) != 0) {
    ...
    if (empty($post_head)
        ...
        || empty($post_tags)
            ...
            try {
                ...
                $_SESSION['success_message'] = "Your post '{$post_head}' has successfully created. Please <a href='./admin_posts.php'>View All Posts page</a></div>";
...
<div class="form-group">
    ...
    <select name="post_category_id" id="post_category">
        ...
        if (isset($cms_pdo) && !empty($cms_pdo)) {
            ...
            foreach ($categories as $cat_admin) {
                echo "<option value='" . $cat_admin['cat_id'] . "'>" . $cat_admin['cat_title'] . "</option>";
            }   
        }
        ?>
    </select>
...
<div class="form-group">
    <input type="submit" class="btn btn-success" name="submit" value="Publish Post">
</div>
</form>
